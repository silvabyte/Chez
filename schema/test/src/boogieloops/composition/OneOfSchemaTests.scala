...
object OneOfSchemaTests extends TestSuite {

  val tests = Tests {
    test("json schema generation") {
      test("basic oneOf schema") {
        val schema = OneOfSchema(List(
          StringSchema(),
          IntegerSchema(),
          BooleanSchema()
        ))
    ...
    test("OneOf validation behavior") {
      ...
      test("fails when no schemas match") {
        ...
        val hasNoMatchError = result1.errors.exists {
          case boogieloops.schema.ValidationError.CompositionError(msg, _) =>
      ...
      test("fails when multiple schemas match") {
        val schema = OneOfSchema(List(
          ...
          StringSchema(minLength = Some(1)), // also matches non-empty strings
          IntegerSchema()
        ))
    ...
    test("Complex OneOf scenarios") {
      test("oneOf with distinct object schemas") {
        val personSchema = ObjectSchema(
          properties = Map(
            "type" -> StringSchema(const = Some("person")),
            "name" -> StringSchema(),
            "age" -> IntegerSchema()
          ),
        ...
        val companySchema = ObjectSchema(
          properties = Map(
            "type" -> StringSchema(const = Some("company")),
        ...
        val schema = OneOfSchema(List(personSchema, companySchema))
      ...
      test("nested oneOf composition") {
        val innerOneOf = OneOfSchema(List(
          StringSchema(minLength = Some(5)),
          IntegerSchema(minimum = Some(10))
        ))

        val outerOneOf = OneOfSchema(List(
          innerOneOf,
          BooleanSchema(),
          ArraySchema(StringSchema(), minItems = Some(1))
        ))
    ...
    test("Error reporting and paths") {
      ...
      test("composition errors include context") {
        ...
        val compositionError = result.errors.find {
          case boogieloops.schema.ValidationError.CompositionError(_, path) => path == "/api/data"
          case _ => false
        }
    ...
    test("Real-world oneOf patterns") {
      test("discriminated union with type field") {
        val circleSchema = ObjectSchema(
          properties = Map(
            "shape" -> StringSchema(const = Some("circle")),
            "radius" -> NumberSchema(minimum = Some(0))
          ),
          required = Set("shape", "radius")
        )

        val rectangleSchema = ObjectSchema(
          properties = Map(
            "shape" -> StringSchema(const = Some("rectangle")),
        ...
        val shapeSchema = OneOfSchema(List(circleSchema, rectangleSchema))
      ...
      test("API response format selection") {
        val successSchema = ObjectSchema(
          properties = Map(
        ...
        val responseSchema = OneOfSchema(List(successSchema, errorSchema))
    ...
    test("ValidationResult framework integration") {
      ...
      test("ValidationResult.invalid() for multiple matches") {
        val schema = OneOfSchema(List(
          StringSchema(),
          StringSchema(maxLength = Some(100))
        ))
    ...
    test("Edge cases and boundary conditions") {
      test("empty schemas list") {
        val schema = OneOfSchema(List.empty)
      ...
      test("single schema in oneOf") {
        val schema = OneOfSchema(List(StringSchema(minLength = Some(3))))

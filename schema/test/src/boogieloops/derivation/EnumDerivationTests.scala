...
object EnumDerivationSchemaTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("Enum with collections") {
      ...
      test("Set of enums") {
        import boogieloops.schema.derivation.CollectionSchemas.given_Schema_Set
      ...
      test("Map with enum values") {
        import boogieloops.schema.derivation.CollectionSchemas.given_Schema_Map
    ...
    test("Mixed data type enum support (Future Enhancement)") {
      test("Current implementation: EnumSchema supports string enum values") {
        ...
        val stringEnumSchema = Schema.StringEnum("red", "amber", "green")

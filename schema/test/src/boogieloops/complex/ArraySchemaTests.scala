...
object ArraySchemaTests extends TestSuite {
  ...
  val tests = Tests {
    test("json schema generation") {
      ...
      test("array with item constraints") {
        val schema = ArraySchema(
          items = IntegerSchema(minimum = Some(0)),
          minItems = Some(1),
          maxItems = Some(10),
    ...
    test("json schema 2020-12 features") {
      ...
      test("contains validation") {
        val schema = ArraySchema(
          items = StringSchema(),
          contains = Some(StringSchema(pattern = Some("^test.*"))),
          minContains = Some(1),
          maxContains = Some(3)
        )
        val json = schema.toJsonSchema
    ...
    test("validation behavior") {
      test("valid array validation") {
        val schema = ArraySchema(StringSchema(), minItems = Some(1), maxItems = Some(3))
        val result =
      ...
      test("min items validation") {
        val schema = ArraySchema(StringSchema(), minItems = Some(2))
        val result = schema.validate(ujson.Arr(ujson.Str("test1")), ValidationContext())
        assert(result.errors.length == 1)
        assert(result.errors.head.isInstanceOf[boogieloops.schema.ValidationError.MinItemsViolation])
      }

      test("max items validation") {
        val schema = ArraySchema(StringSchema(), maxItems = Some(2))
        val result = {
          schema.validate(
        ...
        assert(result.errors.length == 1)
        assert(result.errors.head.isInstanceOf[boogieloops.schema.ValidationError.MaxItemsViolation])
      }

      test("unique items validation") {
        val schema = ArraySchema(StringSchema(), uniqueItems = Some(true))
        val result =
          schema.validate(ujson.Arr(ujson.Str("test1"), ujson.Str("test1")), ValidationContext())
        assert(result.errors.length == 1)
        assert(result.errors.head.isInstanceOf[boogieloops.schema.ValidationError.UniqueViolation])
      }

      test("multiple validation errors") {
        val schema =
          ArraySchema(StringSchema(), minItems = Some(3), maxItems = Some(2), uniqueItems = Some(true))
        val result =
          schema.validate(ujson.Arr(ujson.Str("test1"), ujson.Str("test1")), ValidationContext())
    ...
    test("examples serialization") {
      ...
      test("array examples serialize as valid JSON array") {
        val schema = ArraySchema(IntegerSchema())
          .withExamples(
            ujson.Arr(1, 2, 3),
            ujson.Arr(4, 5, 6)
          )
      ...
      test("withExamples calls work without errors") {
        ...
        val schema1 = ArraySchema(StringSchema()).withExamples(ujson.Arr("test"))
        val schema2 = ArraySchema(IntegerSchema()).withExamples(ujson.Arr(1, 2), ujson.Arr(3, 4))
        val schema3 = ArraySchema(BooleanSchema()).withExamples(ujson.Arr(true, false))
    ...
    test("Tuple validation with prefixItems") {
      test("valid tuple validation") {
        val schema = ArraySchema(
          items = StringSchema(), // fallback for items beyond prefix
      ...
      test("tuple validation with prefix constraint violations") {
        val schema = ArraySchema(
          items = StringSchema(),
          prefixItems = Some(List(
            IntegerSchema(minimum = Some(10)), // First item must be >= 10
            StringSchema(minLength = Some(5)), // Second item must have length >= 5
            BooleanSchema()
          ))
        )
        ...
        val hasIndexZeroError = result.errors.exists(error =>
          error.asInstanceOf[boogieloops.schema.ValidationError].toString.contains("/0")
        )
        val hasIndexOneError = result.errors.exists(error =>
          error.asInstanceOf[boogieloops.schema.ValidationError].toString.contains("/1")
        )
      ...
      test("tuple validation with items beyond prefix") {
        val schema = ArraySchema(
          items = StringSchema(minLength = Some(3)), // Items beyond prefix must be strings >= 3 chars
          prefixItems = Some(List(IntegerSchema(), BooleanSchema()))
        )
    ...
    test("Contains validation") {
      test("valid contains validation") {
        val schema = ArraySchema(
          items = StringSchema(),
          contains = Some(StringSchema(pattern =
            Some("^test.*")
      ...
      test("contains validation - too few matches") {
        ...
        assert(!result.isValid)
        assert(result.errors.length == 1)
        assert(result.errors.head.isInstanceOf[boogieloops.schema.ValidationError.ContainsViolation])

        val containsError = result.errors.head.asInstanceOf[boogieloops.schema.ValidationError.ContainsViolation]
    ...
    test("Complex array validation scenarios") {
      test("array with both prefixItems and contains validation") {
        val schema = ArraySchema(
          ...
          contains = Some(StringSchema(pattern = Some("^valid.*"))),
          minContains = Some(1),
          minItems = Some(3),
          maxItems = Some(6)
        )
      ...
      test("array with multiple constraint violations") {
        val schema = ArraySchema(
          items = StringSchema(minLength = Some(5)),
          prefixItems = Some(List(IntegerSchema(minimum = Some(100)))),
          contains = Some(StringSchema(pattern = Some("^required.*"))),
          minContains = Some(1),
          minItems = Some(5),
          uniqueItems = Some(true)
        )

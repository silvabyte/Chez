...
object StringSchemaTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("min length validation") {
      val schema = StringSchema(minLength = Some(5))
      val result1 = schema.validate(ujson.Str("hello"), ValidationContext())
      assert(result1.isValid)
    ...
    test("max length validation") {
      val schema = StringSchema(maxLength = Some(10))
    ...
    test("pattern validation") {
      val schema = StringSchema(pattern = Some("^[a-zA-Z]+$"))
      val result1 = schema.validate(ujson.Str("hello"), ValidationContext())
      assert(result1.isValid)
    ...
    test("const validation") {
      val schema = StringSchema(const = Some("exactly this"))
    ...
    test("enum validation - moved to EnumSchema") {
      // Note: enum validation is now handled by EnumSchema, not StringSchema
      // This test exists for backward compatibility documentation
      val enumSchema = boogieloops.schema.primitives.EnumSchema.fromStrings("red", "green", "blue")
    ...
    test("email format validation") {
      val schema = StringSchema(format = Some("email"))
    ...
    test("uri format validation") {
      val schema = StringSchema(format = Some("uri"))
    ...
    test("uuid format validation") {
      val schema = StringSchema(format = Some("uuid"))
      val result1 =
    ...
    test("date format validation") {
      val schema = StringSchema(format = Some("date"))
    ...
    test("time format validation") {
      val schema = StringSchema(format = Some("time"))
    ...
    test("date-time format validation") {
      val schema = StringSchema(format = Some("date-time"))
    ...
    test("unknown format validation") {
      val schema = StringSchema(format = Some("unknown-format"))
    ...
    test("default value support") {
      val schema = StringSchema().withDefault(ujson.Str("hello"))
      val json = schema.toJsonSchema
      assert(json("default").str == "hello")

      val schemaEmpty = StringSchema(minLength = Some(0)).withDefault(ujson.Str(""))
      val jsonEmpty = schemaEmpty.toJsonSchema
      assert(jsonEmpty("default").str == "")
      assert(jsonEmpty("minLength").num == 0)

      val schemaEmail =
        StringSchema(format = Some("email")).withDefault(ujson.Str("user@example.com"))
      val jsonEmail = schemaEmail.toJsonSchema
      assert(jsonEmail("default").str == "user@example.com")
      assert(jsonEmail("format").str == "email")
    }

    test("StringSchema validates ujson.Str values correctly") {
      val schema = Schema.String(minLength = Some(3), maxLength = Some(10))
    ...
    test("StringSchema rejects non-string ujson.Value types with TypeMismatch error") {
    ...
    test("All existing validation logic works with ujson.Value") {
      test("pattern validation") {
        val schema = Schema.String(pattern = Some("^[A-Z]+$"))
      ...
      test("const validation") {
        val schema = Schema.String(const = Some("expected"))
      ...
      test("format validation") {
        val emailSchema = Schema.String(format = Some("email"))
    ...
    test("ValidationResult is returned with proper valid/invalid state") {
      val schema = Schema.String(minLength = Some(3))
    ...
    test("String validation using new ValidationResult framework") {
      val schema = Schema.String(minLength = Some(3), pattern = Some("^[a-z]+$"))
    ...
    test("Direct schema validation") {
      val schema = Schema.String(const = Some("test"))

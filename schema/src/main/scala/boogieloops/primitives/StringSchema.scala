  ...
  override def validate(value: ujson.Value, context: ValidationContext): ValidationResult = {
    ...
    value match {
      case ujson.Str(stringValue) =>
        ...
        val minLengthErrors = minLength.fold(List.empty[boogieloops.schema.ValidationError]) { min =>
          if (stringValue.length < min)
            List(boogieloops.schema.ValidationError.MinLengthViolation(min, stringValue.length, context.path))
          else Nil
        }

        val maxLengthErrors = maxLength.fold(List.empty[boogieloops.schema.ValidationError]) { max =>
          if (stringValue.length > max)
            List(boogieloops.schema.ValidationError.MaxLengthViolation(max, stringValue.length, context.path))
          else Nil
        }

        val patternErrors = pattern.fold(List.empty[boogieloops.schema.ValidationError]) { p =>
          if (!stringValue.matches(p))
            List(boogieloops.schema.ValidationError.PatternMismatch(p, stringValue, context.path))
          else Nil
        }

        val constErrors = const.fold(List.empty[boogieloops.schema.ValidationError]) { c =>
          if (stringValue != c)
            List(boogieloops.schema.ValidationError.TypeMismatch(c, stringValue, context.path))
          else Nil
        }

        // Format validation - this is a basic implementation...
        val formatErrors = format.fold(List.empty[boogieloops.schema.ValidationError]) { f =>
          val isValid = f match {
            case "email" =>
          ...
          if (!isValid)
            List(boogieloops.schema.ValidationError.InvalidFormat(f, stringValue, context.path))
          else Nil
        }
      ...
      case _ =>
        ...
        val error = boogieloops.schema.ValidationError.TypeMismatch("string", getValueType(value), context.path)

...
/**
 * Type class for automatically deriving Schema schemas from case class definitions
 *
...
object Schema {
   ...
   */
  def apply[T](using s: Schema[T]): Schema = s.schema
   ...
   */
  given Schema[String] = instance(bl.String())
  given Schema[Int] = instance(bl.Integer())
  given Schema[Long] = instance(bl.Integer())
  given Schema[Double] = instance(bl.Number())
  given Schema[Float] = instance(bl.Number())
  given Schema[Boolean] = instance(bl.Boolean())

  given [T](using s: Schema[T]): Schema[Option[T]] = instance(s.schema.optional)
  given [T](using s: Schema[T]): Schema[List[T]] = instance(bl.Array(s.schema))

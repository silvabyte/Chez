  ...
  def post(
    ...
    Try {
      ...
      case Failure(ex: java.net.http.HttpTimeoutException) =>
        Left(SchemaError.NetworkError(s"Request to $url timed out: ${ex.getMessage}"))
      case Failure(ex: java.net.ConnectException) =>
        Left(SchemaError.NetworkError(s"Failed to connect to $url: ${ex.getMessage}"))
      case Failure(ex) =>
        ...
        Left(SchemaError.NetworkError(errorMsg))
    }
  }
}

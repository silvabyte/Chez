      ...
      try {
        ...
        case _: upickle.core.AbortException | _: ujson.ParsingFailedException =>
          ...
          try {
            ...
            case _: Exception =>
              Left(SchemaError.ParseError(s"Failed to parse Anthropic response: $responseBody"))
          }
      }
    } catch {
      case ex: ujson.ParsingFailedException =>
        Left(SchemaError.ParseError(s"Failed to parse Anthropic response: ${ex.getMessage}"))
      case ex: NoSuchElementException =>
        Left(
          SchemaError.ParseError(s"Missing required field in Anthropic response: ${ex.getMessage}")
        )
      case ex: Exception =>
        Left(SchemaError.ParseError(s"Unexpected error parsing Anthropic response: ${ex.getMessage}"))
    }
  }
      ...
      try {
        ...
        case _: upickle.core.AbortException | _: ujson.ParsingFailedException =>
          ...
          try {
            ...
            case _: Exception =>
              Left(
                SchemaError.ParseError(s"Failed to parse Anthropic object response: $responseBody")
              )
          }
      }
    } catch {
      case ex: ujson.ParsingFailedException =>
        Left(SchemaError.ParseError(s"Failed to parse Anthropic object response: ${ex.getMessage}"))
      case ex: NoSuchElementException =>

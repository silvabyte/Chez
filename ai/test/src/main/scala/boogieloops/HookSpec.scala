...
import boogieloops.ai.{
  ...
  PostObjectResponseHook,
  PostResponseContext,
  PostResponseHook,
  PreObjectRequestContext,
  PreObjectRequestHook,
  PreRequestContext,
  PreRequestHook,
  RequestMetadata,
  Role,
  ...
  val tests = Tests {
    ...
    test("ErrorHook is executed on errors") {
      ...
      class ErrorProvider extends LLMProvider {
        ...
        override def chat(request: ChatRequest): Either[SchemaError, ChatResponse] = {
          Left(SchemaError.NetworkError("Simulated network error", Some(500)))
        }

        override def generateObject(request: ObjectRequest)
            : Either[SchemaError, ObjectResponse[ujson.Value]] = {
          Left(SchemaError.ApiError("Simulated API error", Some("TEST_ERROR"), Some(400)))
        }
      ...
      assert(testHook.errorCalls.head.error.isInstanceOf[SchemaError.NetworkError])
      ...
      assert(testHook.errorCalls.head.error.isInstanceOf[SchemaError.ApiError])
    }
    ...
    test("HookRegistry can add and execute hooks of all types") {
      ...
      val errorContext = ErrorContext(
        agentName = "TestAgent",
        model = "test-model",
        error = SchemaError.NetworkError("test error", None),
        metadata = defaultMetadata,
    ...
    test("Error context includes request information") {
      class ErrorProvider extends LLMProvider {
        ...
        override def chat(request: ChatRequest): Either[SchemaError, ChatResponse] = {
          Left(SchemaError.NetworkError("Network failed", Some(503)))
        }

        override def generateObject(request: ObjectRequest)
            : Either[SchemaError, ObjectResponse[ujson.Value]] = {
          Left(SchemaError.ApiError("Too many requests", Some("RATE_LIMIT"), Some(429)))
        }

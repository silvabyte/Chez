...
import boogieloops.ai.providers.LLMProvider
...
class Agent(config: AgentConfig, @unused initialHistory: Vector[ChatMessage] = Vector.empty)
  ...
  def generateObject[T: Schematic: Reader: scala.reflect.ClassTag](
      userChatMessage: String,
      metadata: RequestMetadata
  )
      : Either[SchemaError, ObjectResponse[T]] = {
    ...
    val schema = summon[Schematic[T]].schema
      ...
      case Right(jsonResponse) =>
        try {
          ...
          case ex: ujson.ParsingFailedException =>
            val error = SchemaError.SchemaConversionError(
  ...
  def generateObjectWithoutHistory[T: Schematic: Reader: scala.reflect.ClassTag](
      userChatMessage: String,
      metadata: RequestMetadata
  ): Either[SchemaError, ObjectResponse[T]] = {
    logger.info(
  ...
  ): Either[SchemaError, EmbeddingResponse] = {

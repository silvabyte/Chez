      ...
      try {
        ...
        case _: upickle.core.AbortException | _: ujson.ParsingFailedException =>
          ...
          try {
            ...
            case _: Exception =>
              Left(SchemaError.ParseError(s"Failed to parse OpenAI response: $responseBody"))
          }
      }
    } catch {
      case ex: ujson.ParsingFailedException =>
        Left(SchemaError.ParseError(s"Failed to parse OpenAI response: ${ex.getMessage}"))
      case ex: NoSuchElementException =>
        Left(SchemaError.ParseError(s"Missing required field in OpenAI response: ${ex.getMessage}"))
      case ex: Exception =>
        Left(SchemaError.ParseError(s"Unexpected error parsing OpenAI response: ${ex.getMessage}"))
    }
  }
      ...
      try {
        ...
        case _: upickle.core.AbortException | _: ujson.ParsingFailedException =>
          ...
          try {
            ...
            case _: Exception =>
              Left(SchemaError.ParseError(s"Failed to parse OpenAI object response: $responseBody"))
          }
      }
    } catch {
      case ex: ujson.ParsingFailedException =>
        Left(SchemaError.ParseError(s"Failed to parse OpenAI response body: ${ex.getMessage}"))
      case ex: NoSuchElementException =>
        Left(SchemaError.ParseError(s"Missing required field in OpenAI response: ${ex.getMessage}"))
      case ex: Exception =>

...
object OpenAIExample extends App with Logging:
  ...
  def exampleStructuredOutput(): Unit = {
    ...
    case class Recipe(
        @Schematic.description("Recipe name") name: String,
        @Schematic.description("List of ingredients") ingredients: List[String],
        @Schematic.description("Cooking time in minutes") @Schematic.minimum(1) cookingTime: Int,
        @Schematic.description("Difficulty level") difficulty: String,
        @Schematic.description("Step-by-step instructions") steps: List[String]

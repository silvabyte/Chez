...
import boogieloops.ai.providers.{OpenAICompatibleProvider, HttpVersion}
...
object EmbeddingSpec extends TestSuite:
  ...
  val tests = Tests {
    ...
    test("Agent validates provider embedding support") {
      ...
      val mockProvider = new boogieloops.ai.providers.LLMProvider {
        val name = "MockProvider"
        val supportedModels = List("mock-model")
        override val supportsEmbeddings = false

        def chat(request: ChatRequest) = Left(SchemaError.ConfigurationError("Not implemented"))
        def generateObject(request: ObjectRequest) =
          Left(SchemaError.ConfigurationError("Not implemented"))
      ...
      result match {
        case Left(SchemaError.ConfigurationError(msg)) =>

...
trait AgentMetrics {
  ...
  def recordRequestStart(agentName: String, model: String, operation: String, metadata: RequestMetadata): Unit
  
  /** Record a successful response */
  def recordSuccess(agentName: String, model: String, operation: String, duration: Long, tokens: Int, metadata: RequestMetadata): Unit
  
  /** Record an error */
  def recordError(agentName: String, model: String, operation: String, error: ChezError, metadata: RequestMetadata): Unit
  
  /** Record history operation */
  def recordHistoryOperation(agentName: String, operation: HistoryOperation, metadata: RequestMetadata): Unit
...
class DefaultAgentMetrics extends AgentMetrics {
  ...
  private val operationMetrics = TrieMap[String, TrieMap[String, AtomicReference[OperationMetrics]]]()
  ...
  override def recordRequestStart(agentName: String, model: String, operation: String, metadata: RequestMetadata): Unit = {
    ...
    val scopeRef = scopeMetrics.getOrElseUpdate(scopeKey, new AtomicReference(
      ScopeMetrics(scopeKey = scopeKey, lastActivity = System.currentTimeMillis())
    ))
    scopeRef.updateAndGet(s => s.copy(requestCount = s.requestCount + 1, lastActivity = System.currentTimeMillis()))
  ...
  override def recordSuccess(agentName: String, model: String, operation: String, duration: Long, tokens: Int, metadata: RequestMetadata): Unit = {
    getOrCreateAgent(agentName)
    ...
    modelRef.updateAndGet { current =>
      val updatedOps = current.operations.get(operation) match {
        case Some(opMetrics) => current.operations.updated(operation, 
          opMetrics.copy(
        ...
        case None => current.operations + (operation -> OperationMetrics(
          count = 1, successCount = 1, totalDuration = duration, totalTokens = tokens,
    ...
    scopeRef.updateAndGet { current =>
      ...
      val newAvgTime = (current.averageResponseTime * (newMessageCount - 1) + duration) / newMessageCount
      current.copy(
  ...
  override def recordError(agentName: String, model: String, operation: String, error: ChezError, metadata: RequestMetadata): Unit = {
    getOrCreateAgent(agentName)
    ...
    val opMap = operationMetrics(agentName)
    val opRef = opMap.getOrElseUpdate(operation, new AtomicReference(OperationMetrics()))
    opRef.updateAndGet(current => current.copy(count = current.count + 1, errorCount = current.errorCount + 1))
    
    // Track error details
    val errorType = error.getClass.getSimpleName
    val errorMap = errorTracking(agentName)
    val errorRef = errorMap.getOrElseUpdate(errorType, new AtomicReference(
      ErrorMetrics(errorType, 0, System.currentTimeMillis(), error.toString)
    ))
    errorRef.updateAndGet(current => current.copy(
      count = current.count + 1,
    ...
    scopeRef.updateAndGet(current => current.copy(
      errorCount = current.errorCount + 1,
      lastActivity = System.currentTimeMillis()
    ))
  }
  
  override def recordHistoryOperation(agentName: String, operation: HistoryOperation, metadata: RequestMetadata): Unit = {
  ...
  override def getSnapshot(agentName: String): Option[AgentMetricsSnapshot] = {
    ...
    val avgDuration = if (successfulRequests > 0) totalDurations(agentName).get().toDouble / successfulRequests else 0.0
    val successRate = if (totalRequests > 0) successfulRequests.toDouble / totalRequests else 0.0
    
    // Get operation metrics
    val opMetrics = operationMetrics.getOrElse(agentName, TrieMap.empty).view.mapValues(_.get()).toMap
...
class MetricsHook(metrics: AgentMetrics) extends PreRequestHook with PostResponseHook 
  ...
  override def onPostResponse(context: PostResponseContext): Unit = {
    context.response match {
      case Right(response) =>
        ...
        metrics.recordSuccess(context.agentName, context.model, "generateText", context.duration, tokens, context.metadata)
      case Left(error) =>
        metrics.recordError(context.agentName, context.model, "generateText", error, context.metadata)
    }
  }
  ...
  override def onPostObjectResponse(context: PostObjectResponseContext): Unit = {
    context.response match {
      case Right(response) =>
        val tokens = response.usage.map(_.totalTokens).getOrElse(0)
        metrics.recordSuccess(context.agentName, context.model, "generateObject", context.duration, tokens, context.metadata)
      case Left(error) =>
        metrics.recordError(context.agentName, context.model, "generateObject", error, context.metadata)
    }
  }
...
object MetricsFactory {
  ...
  def createAnthropicAgentWithMetrics(
    ...
    AgentFactory.createAnthropicAgent(name, instructions, apiKey, model, temperature, maxTokens, hooks)
      .map(_ -> metricsInstance)
  }

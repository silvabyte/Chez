  ...
  val tests = Tests {
    ...
    test("Error metrics track different error types") {
      ...
      metrics.recordError(
        ...
        model,
        "generateText",
        ChezError.NetworkError("Network timeout", Some(408)),
        metadata
      )
      ...
      metrics.recordError(
        ...
        model,
        "generateObject",
        ChezError.ApiError("Rate limited", Some("RATE_LIMIT"), Some(429)),
        metadata
      )
      ...
      metrics.recordError(
        ...
        model,
        "generateText",
        ChezError.NetworkError("Connection failed", Some(503)),
        metadata
      )
    ...
    test("MetricsHook handles errors correctly") {
      ...
      class ErrorProvider extends LLMProvider {
        ...
        override def chat(request: ChatRequest): Either[ChezError, ChatResponse] = {
          Left(ChezError.NetworkError("Simulated error", Some(500)))
        }

        override def generateObject(request: ObjectRequest)
            : Either[ChezError, ObjectResponse[ujson.Value]] = {
          Left(ChezError.ApiError("API error", Some("TEST_ERROR"), Some(400)))
        }

...
object NumberSchemaTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("minimum validation") {
      val schema = NumberSchema(minimum = Some(0.0))
    ...
    test("maximum validation") {
      val schema = NumberSchema(maximum = Some(100.0))
    ...
    test("exclusive minimum validation") {
      val schema = NumberSchema(exclusiveMinimum = Some(0.0))
    ...
    test("exclusive maximum validation") {
      val schema = NumberSchema(exclusiveMaximum = Some(10.0))
    ...
    test("multiple of validation") {
      val schema = NumberSchema(multipleOf = Some(0.5))
    ...
    test("const validation") {
      val schema = NumberSchema(const = Some(3.14))
    ...
    test("enum validation - moved to EnumSchema") {
      // Note: enum validation is now handled by EnumSchema, not NumberSchema
      // This test exists for backward compatibility documentation
      val enumSchema = boogieloops.schema.primitives.EnumSchema.fromNumbers(1.0, 2.5, 5.0)
    ...
    test("json schema generation") {
      val schema = NumberSchema(minimum = Some(0.0), maximum = Some(1.0))
    ...
    test("default value support") {
      val schema = NumberSchema().withDefault(ujson.Num(3.14))
      val json = schema.toJsonSchema
      assert(json("default").num == 3.14)

      val schemaZero = NumberSchema(minimum = Some(0.0)).withDefault(ujson.Num(0.0))
      val jsonZero = schemaZero.toJsonSchema
      assert(jsonZero("default").num == 0.0)
      assert(jsonZero("minimum").num == 0.0)

      val schemaNegative = NumberSchema().withDefault(ujson.Num(-1.5))

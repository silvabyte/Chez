...
object StringSchemaTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("StringSchema validates ujson.Str values correctly") {
      val schema = bl.String(minLength = Some(3), maxLength = Some(10))
    ...
    test("All existing validation logic works with ujson.Value") {
      test("pattern validation") {
        val schema = bl.String(pattern = Some("^[A-Z]+$"))
      ...
      test("const validation") {
        val schema = bl.String(const = Some("expected"))
    ...
    test("Error paths are correctly set using ValidationContext") {
      val schema = bl.String(minLength = Some(5))
    ...
    test("ValidationResult is returned with proper valid/invalid state") {
      val schema = bl.String(minLength = Some(3))
    ...
    test("String validation using new ValidationResult framework") {
      val schema = bl.String(minLength = Some(3), pattern = Some("^[a-z]+$"))
    ...
    test("Direct schema validation") {
      val schema = bl.String(const = Some("test"))

...
object EnumDerivationSchemaTests extends TestSuite {

  // Simple Scala 3 enum
  enum Status derives Schematic {
    case Active, Inactive, Pending
  }

  // Enum with constructor parameters
  enum Priority(val level: Int) derives Schematic {
  ...
  enum Color(val hex: String) derives Schematic {
  ...
  // Single case enum
  enum SingleCase derives Schematic {
    case OnlyOne
  }
  ...
  val tests = Tests {
    ...
    test("Edge case enums") {
      test("Single case enum") {
        val schema = Schematic[SingleCase]
        val json = schema.toJsonSchema
    ...
    test("Enum integration with case classes") {
      test("Case class with single enum field") {
        case class User(name: String, status: Status) derives Schematic

        val schema = Schematic[User]
        val json = schema.toJsonSchema
      ...
      test("Case class with multiple enum fields") {
        case class Task(title: String, priority: Priority, status: Status) derives Schematic

        val schema = Schematic[Task]
        val json = schema.toJsonSchema
      ...
      test("Case class with optional enum field") {
        case class Profile(name: String, priority: Option[Priority]) derives Schematic

        val schema = Schematic[Profile]
        val json = schema.toJsonSchema
    ...
    test("Enum with collections") {
      test("List of enums") {
        case class TaggedItem(name: String, statuses: List[Status]) derives Schematic

        val schema = Schematic[TaggedItem]
        val json = schema.toJsonSchema
      ...
      test("Set of enums") {
        import boogieloops.schema.derivation.CollectionSchemas.given_Schematic_Set
        val _ = summon[Schematic[Set[Status]]]
        case class UniqueStatuses(name: String, statuses: Set[Status]) derives Schematic

        val schema = Schematic[UniqueStatuses]
        val json = schema.toJsonSchema
      ...
      test("Vector of enums") {
        ...
        val _ = summon[Schematic[Vector[Priority]]]
        case class OrderedStatuses(name: String, statuses: Vector[Priority]) derives Schematic

        val schema = Schematic[OrderedStatuses]
        val json = schema.toJsonSchema
      ...
      test("Map with enum values") {
        import boogieloops.schema.derivation.CollectionSchemas.given_Schematic_Map
        val _ = summon[Schematic[Map[String, Status]]]
        case class StatusMap(name: String, userStatuses: Map[String, Status]) derives Schematic

        val schema = Schematic[StatusMap]
        val json = schema.toJsonSchema
    ...
    test("Nested enum structures") {
      test("Case class with nested case class containing enum") {
        case class UserProfile(name: String, settings: UserSettings) derives Schematic
        case class UserSettings(theme: Color, priority: Priority) derives Schematic
        val _ = summon[Schematic[UserSettings]]

        val schema = Schematic[UserProfile]
        val json = schema.toJsonSchema
      ...
      test("Option of case class with enum") {
        case class MaybeSettings(name: String, settings: Option[UserSettingsWithEnum])
            derives Schematic
        case class UserSettingsWithEnum(theme: Color) derives Schematic
        val _ = summon[Schematic[UserSettingsWithEnum]]

        val schema = Schematic[MaybeSettings]
        val json = schema.toJsonSchema
    ...
    test("Mixed data type enum support (Future Enhancement)") {
      test("Current implementation: EnumSchema supports string enum values") {
        // CURRENT BEHAVIOR: EnumSchema supports proper enum handling
        val stringEnumSchema = bl.StringEnum("red", "amber", "green")
        val json = stringEnumSchema.toJsonSchema

        assert(json("type").str == "string")
        assert(json.obj.contains("enum"))

        // EnumSchema properly supports string values in String enum
        ...
        // This would require a new MixedEnumSchema type that accepts List[ujson.Value]
      }

      test("TODO: MixedEnumSchema for heterogeneous enum values") {
        // PLACEHOLDER: Future implementation would look like this:
        //
        // case class MixedEnumSchema(enumValues: List[ujson.Value]) extends Schema {
        ...
        //
        // Usage:
        // val mixedEnum = MixedEnumSchema(List(
        //   ujson.Str("red"),
        //   ujson.Str("amber"),
        //   ujson.Str("green"),
        //   ujson.Null,
      ...
      test("TODO: Object with mixed enum property support") {
        ...
        //
        // This would require:
        // 1. MixedEnumSchema class (see above)
        // 2. Integration with Schema.Object factory method
        // 3. Proper Schema derivation support
        //
        // Example future usage:
        // val objectSchema = Schema.Object(
        //   properties = Map(
        //     "color" -> MixedEnumSchema(List(
      ...
      test("TODO: Schema derivation for mixed enum types") {
        ...
        val schema = Schematic[TrafficLight]
        val json = schema.toJsonSchema
      ...
      test("Enhancement roadmap for mixed enum support") {
        ...
        //
        // 1. Create MixedEnumSchema class
        //    - Accept List[ujson.Value] for enum values
        //    - Generate {"enum": [...]} without type constraint
        //
        // 2. Add factory methods to Schema object
        //    - Schema.MixedEnum(values: List[ujson.Value])
        ...
        //    - MixedEnumSchema.validate method

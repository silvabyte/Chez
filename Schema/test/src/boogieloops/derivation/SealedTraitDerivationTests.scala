...
object SealedTraitDerivationSchemaTests extends TestSuite {
  ...
  sealed trait SingleVariant derives Schematic
  case class OnlyOne(value: String) extends SingleVariant derives Schematic

  // Sealed trait with mixed parameter types
  sealed trait MixedData derives Schematic
  case class TextData(content: String) extends MixedData derives Schematic
  case class NumberData(value: Double, count: Int) extends MixedData derives Schematic
  case class BooleanData(flag: Boolean) extends MixedData derives Schematic

  // Sealed trait with optional fields
  sealed trait OptionalFields derives Schematic
  case class WithOptional(required: String, optional: Option[Int]) extends OptionalFields
      derives Schematic
  case class AllRequired(name: String, age: Int) extends OptionalFields derives Schematic

  // Nested sealed traits
  sealed trait NestedParent derives Schematic
  case class NestedChild(name: String, data: MixedData) extends NestedParent derives Schematic
  case class SimpleChild(id: Int) extends NestedParent derives Schematic
  ...
  val tests = Tests {
    test("Basic sealed trait discriminated union generation") {
      ...
      test("Single variant sealed trait works correctly") {
        val schema = Schematic[SingleVariant]
        val json = schema.toJsonSchema
    ...
    test("Type discriminator field injection") {
      ...
      test("Empty case class gets type discriminator") {
        val schema = Schematic[WithEmpty]
        val json = schema.toJsonSchema
        val variants = json("oneOf").arr
    ...
    test("Sealed trait schema structure validation") {
      test("Sealed trait generates oneOf, not string enum") {
        val sealedSchema = Schematic[Shape]
    ...
    test("Complex sealed trait scenarios") {
      test("Mixed parameter types work correctly") {
        val schema = Schematic[MixedData]
        val json = schema.toJsonSchema
        val variants = json("oneOf").arr
      ...
      test("Optional fields are handled correctly") {
        val schema = Schematic[OptionalFields]
        val json = schema.toJsonSchema
        val variants = json("oneOf").arr
      ...
      test("Nested sealed traits work correctly") {
        val schema = Schematic[NestedParent]
        val json = schema.toJsonSchema
        val variants = json("oneOf").arr
    ...
    test("Integration with case classes") {
      test("Case class containing sealed trait field") {
        case class Container(name: String, shape: Shape) derives Schematic

        val schema = Schematic[Container]
        val json = schema.toJsonSchema
      ...
      test("Optional sealed trait field") {
        case class OptionalContainer(name: String, shape: Option[Shape]) derives Schematic

        val schema = Schematic[OptionalContainer]
        val json = schema.toJsonSchema
    ...
    test("Individual case class schema derivation") {
      ...
      test("Rectangle case class generates correct object schema") {
        val schema = Schematic[Rectangle]
        val json = schema.toJsonSchema
      ...
      test("NumberData case class with multiple fields") {
        val schema = Schematic[NumberData]
        val json = schema.toJsonSchema
      ...
      test("BooleanData case class with boolean field") {
        val schema = Schematic[BooleanData]
        val json = schema.toJsonSchema
      ...
      test("WithOptional case class handles optional fields correctly") {
        val schema = Schematic[WithOptional]
        val json = schema.toJsonSchema
      ...
      test("AllRequired case class has all fields required") {
        val schema = Schematic[AllRequired]
        val json = schema.toJsonSchema
      ...
      test("EmptyCase generates minimal object schema") {
        val schema = Schematic[EmptyCase]
        val json = schema.toJsonSchema
      ...
      test("NonEmptyCase has correct string field") {
        val schema = Schematic[NonEmptyCase]
        val json = schema.toJsonSchema
    ...
    test("Case class vs sealed trait schema differences") {
      ...
      test("Case class compilation and instantiation works") {
        ...
        val rectangleSchema = summon[Schematic[Rectangle]]
        val triangleSchema = summon[Schematic[Triangle]]

...
object IfThenElseSchemaTests extends TestSuite {

  val tests = Tests {
    test("json schema generation") {
      test("basic if-then-else schema") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("type" -> StringSchema(const = Some("user")))),
          thenSchema =
            Some(ObjectSchema(properties = Map("role" -> StringSchema()), required = Set("role"))),
          elseSchema = Some(ObjectSchema(properties = Map("permissions" -> ArraySchema(StringSchema()))))
        )
      ...
      test("if-then schema without else") {
        val schema = IfThenElseSchema(
          condition = StringSchema(pattern = Some("^admin")),
          thenSchema = Some(StringSchema(minLength = Some(10)))
        )
        val json = schema.toJsonSchema
      ...
      test("if-else schema without then") {
        val schema = IfThenElseSchema(
          condition = NumberSchema(minimum = Some(18)),
          elseSchema = Some(NumberSchema(maximum = Some(17)))
        )
        val json = schema.toJsonSchema
      ...
      test("if-then-else schema with metadata") {
        val schema = IfThenElseSchema(
          condition = BooleanSchema(),
          thenSchema = Some(StringSchema()),
          elseSchema = Some(IntegerSchema())
    ...
    test("IfThenElse validation behavior") {
      test("applies then schema when condition matches") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("type" -> StringSchema(const = Some("user")))),
          thenSchema =
            Some(ObjectSchema(properties = Map("name" -> StringSchema()), required = Set("name"))),
          elseSchema = Some(ObjectSchema(properties = Map("id" -> IntegerSchema())))
        )
      ...
      test("applies else schema when condition doesn't match") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("type" -> StringSchema(const = Some("admin")))),
          thenSchema =
            Some(ObjectSchema(
              properties = Map("permissions" -> ArraySchema(StringSchema())),
      ...
      test("fails when then schema doesn't validate") {
        val schema = IfThenElseSchema(
          ...
          thenSchema = {
            Some(ObjectSchema(
              properties = Map("name" -> StringSchema(minLength = Some(10))),
              required = Set("name")
            ))
          }
        )
      ...
      test("fails when else schema doesn't validate") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("type" -> StringSchema(const = Some("admin")))),
          thenSchema = Some(ObjectSchema(properties = Map("permissions" -> ArraySchema(StringSchema())))),
          elseSchema = {
            Some(ObjectSchema(
              properties = Map("age" -> IntegerSchema(minimum = Some(18))),
              required = Set("age")
            ))
          }
        )
      ...
      test("validates when no then schema and condition matches") {
        val schema = IfThenElseSchema(
          condition = StringSchema(minLength = Some(5)),
          elseSchema = Some(StringSchema(maxLength = Some(3)))
        )
      ...
      test("validates when no else schema and condition doesn't match") {
        val schema = IfThenElseSchema(
          condition = StringSchema(minLength = Some(10)),
          thenSchema = Some(StringSchema(pattern = Some("^[A-Z].*")))
        )
    ...
    test("Complex IfThenElse scenarios") {
      test("nested conditional validation") {
        val innerIfThenElse = IfThenElseSchema(
          condition = StringSchema(pattern = Some("^admin.*")),
          thenSchema = Some(StringSchema(minLength = Some(10))),
          elseSchema = Some(StringSchema(maxLength = Some(8)))
        )

        val outerIfThenElse = IfThenElseSchema(
      ...
      test("object type discrimination") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("type" -> StringSchema(const = Some("circle")))),
          thenSchema = Some(ObjectSchema(
            properties = Map(
              "radius" -> NumberSchema(minimum = Some(0))
            ),
            required = Set("radius")
          )),
          elseSchema = Some(ObjectSchema(
            properties = Map(
      ...
      test("array length conditional validation") {
        val schema = IfThenElseSchema(
          condition = ArraySchema(StringSchema(), minItems = Some(3)),
          thenSchema = Some(ArraySchema(StringSchema(minLength = Some(5)))),
          elseSchema = Some(ArraySchema(StringSchema()))
        )
    ...
    test("Error reporting and paths") {
      ...
      test("error paths are preserved during else validation") {
        val schema = IfThenElseSchema(
          condition = IntegerSchema(),
          elseSchema = Some(StringSchema(minLength = Some(10)))
        )
      ...
      test("condition validation doesn't affect error reporting") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("flag" -> BooleanSchema())),
          thenSchema = Some(StringSchema(minLength = Some(10)))
        )
    ...
    test("Real-world conditional patterns") {
      test("user role-based validation") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("role" -> StringSchema(const = Some("admin")))),
          thenSchema = Some(ObjectSchema(
            properties = Map(
              "permissions" -> ArraySchema(StringSchema(), minItems = Some(1)),
      ...
      test("API versioning conditional validation") {
        val schema = IfThenElseSchema(
          condition = ObjectSchema(properties = Map("version" -> StringSchema(const = Some("v2")))),
          thenSchema = Some(ObjectSchema(
            properties = Map(
              "data" -> ObjectSchema(properties = Map("format" -> StringSchema(const = Some("json"))))
            ),
            required = Set("data")
          )),
          elseSchema = Some(ObjectSchema(
      ...
      test("feature flag conditional validation") {
        val schema = IfThenElseSchema(
          condition =
            ObjectSchema(properties = Map("experimentalFeatures" -> BooleanSchema(const = Some(true)))),
          thenSchema = Some(ObjectSchema(properties = {
            Map(
              "betaConfig" -> ObjectSchema(properties = Map("enabled" -> BooleanSchema()))
            )
          })),
          elseSchema = Some(ObjectSchema(properties = {
            Map(
              "stableConfig" -> ObjectSchema(properties = Map("version" -> StringSchema()))
            )
          }))
        )
    ...
    test("ValidationResult framework integration") {
      test("ValidationResult.valid() for successful conditional validation") {
        val schema = IfThenElseSchema(
          condition = StringSchema(),
          thenSchema = Some(StringSchema(minLength = Some(1)))
        )
        val value = ujson.Str("test")
        val context = ValidationContext()
        val result = schema.validate(value, context)
    ...
    test("Edge cases and boundary conditions") {
      test("condition only (no then or else)") {
        val schema = IfThenElseSchema(condition = StringSchema())
        val value = ujson.Str("test")
        val context = ValidationContext()
        val result = schema.validate(value, context)
      ...
      test("complex condition evaluation") {
        val complexCondition = AllOfSchema(List(
          StringSchema(minLength = Some(5)),
          StringSchema(pattern = Some("^[A-Z].*"))
        ))
        val schema = IfThenElseSchema(
          condition = complexCondition,
          thenSchema = Some(StringSchema(maxLength = Some(10))),
          elseSchema = Some(StringSchema())
        )
      ...
      test("condition with type mismatch") {
        val schema = IfThenElseSchema(
          ...
          thenSchema = Some(ObjectSchema(properties = Map("key" -> StringSchema()))),
          elseSchema = Some(StringSchema())
        )

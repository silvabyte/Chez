  ...
  def applyMetadata(baseSchema: Schema, metadata: AnnotationMetadata): Schema = {
    ...
    metadata.enumValues match {
      case Some(values) =>
        ...
        var result: Schema = EnumSchema(ujsonValues)
        // scalafix:on DisableSyntax.var
    ...
    var result = baseSchema match {
      case sc: StringSchema =>
        ...
        metadata.const.foreach {
          ...
          case _ => // Non-string const values don't apply to StringSchema
        }
        enhanced

      case nc: NumberSchema =>
        // scalafix:off DisableSyntax.var
        ...
        metadata.const.foreach {
          ...
          case _ => // Non-numeric const values don't apply to NumberSchema
        }
        enhanced

      case ic: IntegerSchema =>
        // scalafix:off DisableSyntax.var
        ...
        metadata.const.foreach {
          ...
          case _ => // Non-numeric const values don't apply to IntegerSchema
        }
        enhanced

      case ac: ArraySchema[_] =>
        // scalafix:off DisableSyntax.var

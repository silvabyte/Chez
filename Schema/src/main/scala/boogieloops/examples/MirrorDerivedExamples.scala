...
import boogieloops.schema.derivation.{Schematic, ValidatedReadWriter}

/**
 * This showcases Mirror-Based Schema Derivation enabling the syntax: case class User derives Schematic
 */
object MirrorDerivedExamples {
  ...
  def basicDerivedSchemaExample(): Unit = {
    ...
    // Define case class with derives Schematic
    case class User(id: String, name: String, age: Option[Int] = None) derives Schematic, ReadWriter

    // Get the automatically derived schema
    val userSchema = Schematic[User]
  ...
  def nestedCaseClassExample(): Unit = {
    ...
    case class Address(street: String, city: String, zipCode: String) derives Schematic, ReadWriter
    case class Person(name: String, address: Address, contacts: List[String]) derives Schematic,
          ReadWriter

    val addressSchema = Schematic[Address]
    val personSchema = Schematic[Person]
  ...
  def optionalFieldsExample(): Unit = {
    ...
    val profileSchema = Schematic[Profile]
  ...
  def validatedReadWriterExample(): Unit = {
    ...
    case class Product(id: String, name: String, price: Double) derives Schematic, ReadWriter

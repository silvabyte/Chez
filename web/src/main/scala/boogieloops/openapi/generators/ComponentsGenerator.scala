...
object ComponentsGenerator {
  ...
  private def extractSchemas(allRoutes: Map[String, RouteSchema])
    ...
    if (namedSchemas.nonEmpty) {
      Some(namedSchemas.map { case (name, schema) =>
        name -> schema.toJsonSchema // Direct JSON Schema, no wrapper
      })
    } else None
  }
   ...
   */
  private def deduplicateSchemas(schemas: List[Schema]): List[Schema] = {
    schemas
   ...
   */
  private def generateSchemaNames(schemas: List[Schema]): Map[String, Schema] = {

...
object PathsGenerator {
  ...
  private def convertToOperation(
    ...
    OperationObject(
      ...
      operationId = if (config.generateOperationIds)
        Some(generateOperationId(method, path))
      else None,
   ...
   */
  private def convertQueryParameters(querySchema: Schema): List[ParameterObject] = {
   ...
   */
  private def convertHeaderParameters(headersSchema: Schema): List[ParameterObject] = {
   ...
   */
  private def convertToRequestBody(bodySchema: Schema): RequestBodyObject = {
    RequestBodyObject(
   ...
   */
  private def convertHeadersSchema(headersSchema: Schema): Map[String, HeaderObject] = {
   ...
   */
  private def convertSecurity(security: List[SecurityRequirement])
      : Option[List[SecurityRequirementObject]] = {
  ...
  private def extractCommonParameters(
      path: String,
      methodRoutes: Map[String, RouteSchema]
  ): Option[List[ParameterObject]] = {

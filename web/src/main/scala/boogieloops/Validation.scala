...
object ValidationError {
  ...
  def fromSchemaError(chezError: boogieloops.schema.ValidationError, context: String): ValidationError = {
    ...
    chezError match {
      ...
      case boogieloops.schema.ValidationError.InvalidFormat(format, value, path) =>
        errorType(s"Invalid format '$format' for value: $value", path, None)
      case boogieloops.schema.ValidationError.OutOfRange(min, max, value, path) =>
      ...
      case boogieloops.schema.ValidationError.ParseError(message, path) =>
        errorType(s"Parse error: $message", path, None)
      case boogieloops.schema.ValidationError.PatternMismatch(pattern, value, path) =>
        errorType(s"Value '$value' does not match pattern: $pattern", path, None)
      case boogieloops.schema.ValidationError.AdditionalProperty(property, path) =>
        errorType(s"Additional property not allowed: $property", path, Some(property))
      case boogieloops.schema.ValidationError.UniqueViolation(path) =>
        errorType("Array items must be unique", path, None)
      case boogieloops.schema.ValidationError.MinItemsViolation(min, actual, path) =>
        errorType(s"Array must have at least $min items, got $actual", path, None)
      case boogieloops.schema.ValidationError.MaxItemsViolation(max, actual, path) =>
        errorType(s"Array must have at most $max items, got $actual", path, None)
      case boogieloops.schema.ValidationError.ContainsViolation(minContains, maxContains, actualContains, path) =>
      ...
      case boogieloops.schema.ValidationError.MinLengthViolation(min, actual, path) =>
        errorType(s"String must be at least $min characters, got $actual", path, None)
      case boogieloops.schema.ValidationError.MaxLengthViolation(max, actual, path) =>
        errorType(s"String must be at most $max characters, got $actual", path, None)
      case boogieloops.schema.ValidationError.MinPropertiesViolation(min, actual, path) =>
        errorType(s"Object must have at least $min properties, got $actual", path, None)
      case boogieloops.schema.ValidationError.MaxPropertiesViolation(max, actual, path) =>
        errorType(s"Object must have at most $max properties, got $actual", path, None)
      case boogieloops.schema.ValidationError.MultipleOfViolation(multipleOf, value, path) =>
        errorType(s"Value $value must be a multiple of $multipleOf", path, None)
      case boogieloops.schema.ValidationError.CompositionError(message, path) =>
        errorType(s"Composition validation failed: $message", path, None)
      case boogieloops.schema.ValidationError.ReferenceError(ref, path) =>
...
object ValidationHelpers {
  ...
  def validateRequestBody(
    ...
    Try {
      ...
      case Success(bodyJson) =>
        val context = boogieloops.schema.validation.ValidationContext("/body")
        bodySchema.validate(bodyJson, context) match {
          case result if result.isValid => Right(bodyJson)
          case result => Left(result.errors.map(ValidationError.fromSchemaError(_, "body")))
        }
      case Failure(e) =>
  ...
  def validateQueryParams(
    ...
    Try {
      ...
      val context = boogieloops.schema.validation.ValidationContext("/query")
      querySchema.validate(queryJson, context) match {
        case result if result.isValid =>
          Right(queryMap)
        case result =>
          Left(result.errors.map(ValidationError.fromSchemaError(_, "query")))
      }
    } match {
  ...
  def validatePathParams(
    ...
    Try {
      ...
      val context = boogieloops.schema.validation.ValidationContext("/params")
      paramsSchema.validate(paramsJson, context) match {
        case result if result.isValid =>
          Right(paramMap)
        case result =>
          Left(result.errors.map(ValidationError.fromSchemaError(_, "params")))
      }
    } match {
      case Success(result) => result
      case Failure(e) =>
  ...
  def validateHeaders(
    ...
    Try {
      ...
      val context = boogieloops.schema.validation.ValidationContext("/headers")
      headersSchema.validate(headersJson, context) match {
        case result if result.isValid =>
          Right(headerMap)
        case result =>
          Left(result.errors.map(ValidationError.fromSchemaError(_, "headers")))
      }
    } match {
      case Success(result) => result
      case Failure(e) =>

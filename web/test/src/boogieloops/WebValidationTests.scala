  ...
  val tests = Tests {
    ...
    test("Request body validation") {
      ...
      test("invalid JSON body fails validation") {
        val bodySchema = Schema.Object(
          properties = Map("name" -> Schema.String(minLength = Some(1))),
          required = Set("name")
        )
      ...
      test("malformed JSON body fails with parse error") {
        val bodySchema = Schema.Object(properties = Map("name" -> Schema.String()))
    ...
    test("Query parameter validation") {
      test("valid query parameters validate against schema") {
        val querySchema = Schema.Object(
          properties = Map(
            "page" -> Schema.Integer(minimum = Some(1)),
            "limit" -> Schema.Integer(minimum = Some(1), maximum = Some(100))
          ),
          required = Set("page")
        )
      ...
      test("invalid query parameters fail validation") {
        val querySchema = Schema.Object(
          properties = Map("page" -> Schema.Integer(minimum = Some(1))),
          required = Set("page")
        )
    ...
    test("Path parameter validation") {
      test("valid path parameters validate against schema") {
        val paramsSchema = Schema.Object(
          properties = Map(
            "id" -> Schema.Integer(minimum = Some(1)),
            "category" -> Schema.String(minLength = Some(1))
          ),
      ...
      test("invalid path parameters fail validation") {
        val paramsSchema = Schema.Object(
          properties = Map("id" -> Schema.Integer(minimum = Some(1))),
          required = Set("id")
        )
    ...
    test("Header validation") {
      test("valid headers validate against schema") {
        val headersSchema = Schema.Object(
          properties = Map(
            "Authorization" -> Schema.String(pattern = Some("Bearer .*")),
            "Content-Type" -> Schema.String(const = Some("application/json"))
          ),
      ...
      test("invalid headers fail validation") {
        val headersSchema = Schema.Object(
          properties = Map("Authorization" -> Schema.String(pattern = Some("Bearer .*"))),
          required = Set("Authorization")
        )
    ...
    test("Complete request validation") {
      test("valid complete request validates successfully") {
        val routeSchema = RouteSchema(
          body = Some(Schema.Object(
            properties = Map("name" -> Schema.String()),
            required = Set("name")
          )),
          query = Some(Schema.Object(
            properties = Map("page" -> Schema.Integer(minimum = Some(1)))
          )),
          params = Some(Schema.Object(
            properties = Map("id" -> Schema.Integer()),
            required = Set("id")
          )),
          headers = Some(Schema.Object(
            properties = Map("Content-Type" -> Schema.String())
          ))
        )
    ...
    test("String to JSON type conversion") {
      test("converts various string types correctly") {
        ...
        val querySchema = Schema.Object(
          properties = Map(
            "bool_true" -> Schema.Boolean(),
            "bool_false" -> Schema.Boolean(),

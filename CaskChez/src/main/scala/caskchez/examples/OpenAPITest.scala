...
object OpenAPITest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    RouteSchemaRegistry.register("/users", "POST", RouteSchema(
    ...
    RouteSchemaRegistry.register("/users/{id}", "GET", RouteSchema(
    ...
    println(s"Number of components: ${openAPIDoc.components.flatMap(_.schemas).map(_.size).getOrElse(0)}")

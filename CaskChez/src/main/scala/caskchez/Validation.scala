  ...
  def validateRequest(
    ...
    routeSchema.body.foreach { schema =>
      Try {
        ...
        case Failure(e) => allErrors ++= List(ValidationError.RequestBodyError(
            s"Failed to parse body: ${e.getMessage}",
            "/"
          ))
      }
    }
    ...
    if (allErrors.nonEmpty) {
      ...
      val validatedRequest = ValidatedRequest(
        ...
        validatedQuery =
          if (request.queryParams.nonEmpty) Some(request.queryParams.map { case (k, v) =>
            k -> v.headOption.getOrElse("")
          })
          else None,
        validatedHeaders =
          if (request.headers.nonEmpty) Some(request.headers.map { case (k, v) =>
            k -> v.headOption.getOrElse("")
          })
          else None
      )
